...
- By default, up to 100 buckets for each AWS account, can be raised on request to AWS
...
- AWS recommends objects larger than 100MB use multi-part upload
- AWS CLI and high-level APIs use multi-part upload automatically, low-level APIs will need to manage this more granularly.

S3 objects have Eventual Consistency for updates and deletes across replicated buckets, there can be some delay but usually consistent within 2 seconds. This is not a concern for newly created objects though.
...
- Use Route 53 to assign your site a DNS
- Use AWS Certificate Manager to assign a free SSL/TLS certificate in conjunction with CloudFront
...
All classes have 11 nines (99.999999999%) durability guarantee.
...
NOTE: Minimum storage duration is 30 days before you can transition from S3 Standard to S3 IA (One Zone IA or Standard IA)
...
- Transfer files to AWS CloudFront edge locations, to forward to S3 bucket
- Fast S3 transfers over long distances
- Compatible with multi-part upload
- Edge location can use private AWS network which is faster
- Use S3 Transfer Acceleration Speed Comparison tool to see if it will help
...
- Could use KMS-managed Customer Master Key (CMK)
...
- Doesn't log API calls, logs access (which might result from API calls). Use CloudTrail to track API calls.
...
`aws s3 sync` command can be used to copy object between S3 buckets