...
NOTE User Data - runs only during boot cycle when you first launch the instance, but can be configured to run every time you restart. Executes with root user privileges.
...
- No cost when assigned to an instance
...
- Data at rest encrypted, data between EBS and EC2 encrypted, snapshots from EBS also encrypted.
